...
#
# Tags look like `v<Apache NiFi Registry version>-<release>`, the same scheme
# the sibling Apache NiFi role publishes under (e.g. `v2.12.0-0`):
#
# - if defaults/main.yml points at an Apache NiFi Registry version that has never
#   been released, the release counter restarts at 0 (`v2.12.0-0`)
# - otherwise the counter is incremented (`v2.12.0-1`), but only if something
...
# Anchored on `nifi_registry_version:` so that neither the `# renovate:` annotation above
# it nor `nifi_registry_container_image_tag`,
# which are derived from it, can be mistaken for it.
version="$(sed -nE 's|^nifi_registry_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Apache NiFi Registry version from $defaults_path"
	exit 1
fi

# Apache NiFi Registry's own version is carried without a leading `v` (the `v` lives in
